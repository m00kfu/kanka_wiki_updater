...
def mock_state(tmp_path):
    ...
    with patch('kanka_wiki_updater.sync.ingest_journal.state') as mock:
...
@pytest.fixture(autouse=True)
def mock_llm():
    with patch('kanka_wiki_updater.sync.synopsis_generator.chat_json') as mock:
        mock.return_value = {
...
@pytest.fixture(autouse=True)
def mock_client():
    with patch('kanka_wiki_updater.core.kanka_client.KankaClient') as Mock:
        client = MagicMock()
...
class TestLimitHandling:
    @patch('kanka_wiki_updater.sync.ingest_journal.main')
    def test_limit_passes_to_main(self, mock_main):
        from kanka_wiki_updater.sync import ingest_journal

        parser = ingest_journal.argparse.ArgumentParser()
...
class TestIdempotency:
    def test_skips_processed_journals(self):
        from kanka_wiki_updater.sync import ingest_journal as sync_pipeline
...
class TestNoMeaningfulChange:
    def test_same_synopsis_no_relations_returns_none(self, monkeypatch):
        from kanka_wiki_updater.sync.synopsis_generator import propose_update as sp
        ...
        monkeypatch.setattr('kanka_wiki_updater.sync.synopsis_generator.chat_json', lambda sys, usr, **kwargs: mock_result)
        ...
        result = sp(123, entity_data, journal, {})
        assert result is None
