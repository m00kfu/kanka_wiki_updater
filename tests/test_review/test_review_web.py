...
class TestApiProposalStatus:
    def test_update_status_approved_all(self, app_with_queue):
        ...
        with (
            mock.patch('kanka_wiki_updater.review.web.KankaClient', return_value=mock_client),
            mock.patch(
...
@pytest.mark.skip(reason="Frontend HTML/JS removed during refactor")
...
class TestApiProposalSync:
    ...
    def test_sync_endpoint_exists(self, app_with_queue):
        ...
        with (
            mock.patch('kanka_wiki_updater.review.web.KankaClient', return_value=mock_client),
            mock.patch('kanka_wiki_updater.sync.sync_engine.build_entity_index', return_value={}),
        ):
    ...
    def test_sync_failure_returns_error(self, app_with_queue):
        ...
        with mock.patch('kanka_wiki_updater.review.web.KankaClient', return_value=mock_client):
            resp = app_with_queue.post('/api/proposals/0/sync')
            data = resp.get_json()
...
class TestApiProposalRegenerate:
    ...
    def test_regenerate_identical_output_returns_409(self, app_with_queue):
        ...
        with (
            ...
            umock.patch('kanka_wiki_updater.sync.synopsis_generator.chat_json') as mock_chat,
        ):
...
class TestRegenerateProposalFunction:
    ...
    def test_success_returns_ok_with_proposed_entry(self):
        ...
        with umock.patch(
            'kanka_wiki_updater.sync.synopsis_generator.build_synopsis_proposal',
            return_value={
