...
from kanka_wiki_updater.core.kanka_client import KankaClient, KankaError
...
class TestRequestErrors:
    @patch('kanka_wiki_updater.core.kanka_client.requests.Session')
...
class TestRequestRetry:
    ...
    def test_rate_limit_retries_then_succeeds(self, mock_session_cls):
        ...
        with patch('kanka_wiki_updater.core.kanka_client._time') as mock_time:
            client = KankaClient.__new__(KankaClient)
            client._base_url = 'https://api.kanka.io/1.0'
            client._campaign_id = 999
            client._retry_on_rate_limit = True
