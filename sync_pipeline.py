...
from .mentions import strip_html, normalize_text, linked_entity_ids, fuzzy_name_matches
...
def relation_summary(relations, index):
    ...
    for rel in relations:
        ...
        lines.append(f"- {rel['relation']} -> {other_name} (attitude: {rel.get('attitude')})")
    return "\n".join(lines)
...
def propose_update(entity_id, entity, journal, index):
    session_text = strip_html(journal.get("entry"))
    ...
    user_prompt = USER_PROMPT_TEMPLATE.format(
        ...
        journal_name=journal.get("name", "Session note"),
        journal_date=journal.get("date") or journal.get("created_at", ""),
    ...
    return {
        ...
        "proposed_entry": result.get("updated_entry", entity["entry"]),
...
def journal_sort_key(journal):
    ...
    if year is not None:
        return (0, year, journal.get("calendar_month") or 0, journal.get("calendar_day") or 0,
...
def apply_relation_changes_locally(entity_id, relation_changes, index, name_to_id):
    ...
    relations = index[entity_id]["relations"]
...
def main(limit=None):
    ...
    if limit is not None and limit < total_new:
        ...
        print(f"{total_new} journal(s) are new; processing the oldest {len(to_process)} "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Sync new Kanka session journals into proposed wiki updates."
    )
    parser.add_argument(
        "--limit",
        type=int,
