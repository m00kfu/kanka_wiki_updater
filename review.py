...
from .mentions import strip_html, normalize_text, linked_entity_ids, find_unlinked_mentions, add_missing_entity_tags
...
def review_new_entity_proposal(proposal, index, name_to_id, client):
    ...
    choice = prompt_choice(
        "\nCreate in Kanka? (y)es as suggested type / "
        "(c)reate as the other type instead / (n)o, skip",
        choices="ycn",
    )
    if choice == "n":
...
def review_proposal(proposal, index, name_to_id, client, update_num=None, total_updates=None):
    ...
    unlinked_warning = unlinked_mention_warning(proposal["proposed_entry"], index, exclude_entity_id=proposal["entity_id"])
    if unlinked_warning:
        print(colors.yellow(unlinked_warning))
                ...
                if action == "delete":
                    if existing and existing.get("id"):
                        ...
                        relation_results.append({
                            "action_taken": "deleted", "target_id": target_id,
                            "target_name": rc["target_name"], "previous_relation": existing,
                        })
                    elif existing:
                        print(colors.yellow(
                        ...
                        print(colors.dim(f"  (No existing relation to '{rc['target_name']}' found to delete -- nothing to do.)"))

                elif action == "update" and existing and existing.get("id"):
                    client.update_relation(entity_id, existing["id"], relation=rc["relation"], attitude=rc.get("attitude"))
                    print(colors.green(f"  - Updated relation -> {rc['target_name']}: '{rc['relation']}'"))
                    relation_results.append({
                        "action_taken": "updated", "target_id": target_id,
                        "target_name": rc["target_name"], "previous_relation": existing,
                    })
                    ...
                    if body.get("target_id") == target_id:
                        print(colors.green(f"  - Created relation -> {rc['target_name']}: '{rc['relation']}' (confirmed by Kanka)"))
                    else:
                        print(colors.green(
                    ...
                    relation_results.append({
                        "action_taken": "created", "target_id": target_id,
